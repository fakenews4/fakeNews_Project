...
# ✅ Google AI API 설정
genai.configure(api_key="AIzaSyAqiutR9Uc1Q4DUtRQdiAu58U6apM8jmek")

# ✅ 학습된 모델 불러오기
model_path = "C:/FakeNewsProject/FakeNews_py/ModelFolder/datatext_binary_model.h5"
model = tf.keras.models.load_model(model_path)

# ✅ 모델 구조 확인 (sigmoid인지 확인)
model.summary()
...
tokenizer.fit_on_texts(summarized_news_texts)
...
# ✅ 예측값 확인 (확률값 확인)
print("모델 예측 확률값:", predictions.flatten())

# ✅ 확률값 그대로 유지 (이진 변환 X)
is_fake = predictions.flatten()

# ✅ 가짜 뉴스 판별 근거 제공 (LIME 사용)
explainer = LimeTextExplainer(class_names=["진짜 뉴스", "가짜 뉴스"])

def predict_proba(texts):
    sequences = tokenizer.texts_to_sequences(texts)
    processed_input = pad_sequences(sequences, maxlen=max_length, padding="post")
    return model.predict(processed_input)  # 확률값 반환

# ✅ 결과 출력
for i, (original_text, summary, prob) in enumerate(zip(news_texts, summarized_news_texts, is_fake)):
    print(f"🔹 원본 뉴스 {i+1}: {original_text[:100]}...\n")  
    print(f"🔸 요약된 뉴스: {summary[:100]}...\n")
    print(f"⚖ 판별 결과: {'⚠ 가짜 뉴스' if prob > 0.5 else '✔ 진짜 뉴스'} (확률: {prob:.2%})")

    # ✅ LIME 예외 처리
    if prob > 0.1:  # 10% 이상 확률이면 LIME 실행
        try:
...
accuracy = accuracy_score(true_labels, (is_fake > 0.5).astype(int))
precision = precision_score(true_labels, (is_fake > 0.5).astype(int))
recall = recall_score(true_labels, (is_fake > 0.5).astype(int))
f1 = f1_score(true_labels, (is_fake > 0.5).astype(int))
