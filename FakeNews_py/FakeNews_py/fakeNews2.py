...
# ✅ 예측값 확인 (모든 예측값이 0인지 확인)
print("모델 원본 예측값:", predictions.flatten())

# ✅ 확률값 보정 (너무 극단적이지 않게 조정)
def adjust_probabilities(pred, alpha=0.2):
    return (1 - alpha) * pred + alpha * 0.5

adjusted_predictions = adjust_probabilities(predictions.flatten())
...
# ✅ 성능 평가
true_labels = np.array([1, 0, 0, 0, 1, 0])  # 실제 정답
is_fake = (adjusted_predictions > 0.5).astype(int)
...
print("모델 예측 확률값:", model.predict(processed_input))
